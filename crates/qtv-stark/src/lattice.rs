...
/// matrix vector product for one signature, the rows times the columns times
/// the ring degree.
...
/// signature contributes the coordinatewise products of the transform domain
/// matrix vector product. The values stand in for reduced transform outputs and
/// are drawn from a deterministic sequence.
...
/// matrix vector product this carries the matrix coefficient.
pub const COL_A: usize = 0;
/// The second factor column, relative to the piece base. In the matrix vector
/// product this carries the transform domain response coefficient.
