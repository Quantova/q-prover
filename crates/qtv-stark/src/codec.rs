...
//!
//! A proof is a tree of digests and field elements. This module writes it to a
//! flat byte string and reads it back, so a proof can travel as the opaque bytes
//! of a certificate body and be checked by the same verifier without the trace.
//! Decoding is total: any truncated or malformed input returns None rather than
//! panicking, so a proof read off the wire cannot crash the verifier.
...
/// malformed, or carry trailing data.
