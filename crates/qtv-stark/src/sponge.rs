...
/// rounded up to a power of two so the trace length is a power of two.
...
/// lanes of the last absorb permutation output.
...
/// first squeeze block. The blocks fold in at the segment boundaries, so the trace
/// proves the sponge absorbed the whole input, not only the first block.
...
/// permutation, and the absorb selectors mark the boundary rows that fold the next
/// block into the rate lanes.
