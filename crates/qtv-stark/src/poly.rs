...
//!
//! The trace columns are interpolated over a small subgroup and extended onto a
//! larger coset. Both directions use a radix two number theoretic transform that
//! rests on the two adic roots of unity carried by the field.

use crate::field::{root_of_unity, Felt};

/// In place radix two transform. On return values[i] holds the evaluation of the
/// input coefficient vector at omega to the i, where omega is a primitive root
/// of unity of the vector length.
...
/// trick. Every input must be non zero.
