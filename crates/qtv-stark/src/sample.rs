...
//!
//! The public matrix is expanded by squeezing SHAKE128 over the seed and reading
//! the stream three bytes at a time. Each triple forms a candidate integer z of
//! twenty three bits, the low byte, the middle byte, and the low seven bits of the
//! high byte. A candidate is accepted when it is below the signature modulus and
//! becomes the next coefficient, otherwise it is dropped. This is RejNTTPoly of
//! FIPS 204, Algorithm 30.
//!
//! This module arithmetizes the accept decision of every candidate. Each row holds
//! one triple, recomposes the candidate, and carries an accept bit that a range
//! witness pins to the comparison z below q. The stream bytes are validated
//! against the crypto crate SHAKE128, so the trace proves the exact rejection the
//! reference performs. The accepted coefficients read off the trace in order are
//! the ring the matrix expansion produces.
...
/// rejection margin, so the first buffer yields the whole ring with overwhelming
/// probability.
...
/// the reference RejNTTPoly restricted to the first buffer. Returns the accepted
/// coefficients in order, the whole ring when the buffer suffices.
...
/// can be placed inside a wider joined trace.
...
/// one byte triple. The trace is padded with zero triples up to a power of two
/// length, which are accepted since zero is below the modulus but carry no meaning
/// past the ring degree.
