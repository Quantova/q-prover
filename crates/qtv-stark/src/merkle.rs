...
//!
//! Commitments bind the prover to its evaluation vectors. Every digest comes
//! from SHA3 256 in the crypto crate, so the whole tree rests on hashing alone.
...
///
/// The canonical little endian encoding fixes the preimage so that the prover
/// and the verifier agree on every leaf.
...
///
/// The canonical little endian encoding of each element is concatenated in
/// column order, so a whole trace row commits under a single leaf.
