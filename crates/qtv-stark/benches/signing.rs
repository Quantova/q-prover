...
//!
//! This measures the cost of proving one signature's derivation, the mechanism
//! option A rests on. It is a proof over the signing computation, not the
//! verification relation, so it is heavier and it puts the secret key in the
//! witness. The bench arithmetizes the accepted iteration end to end, reusing the
//! transform, the hashing, the norm, and the decomposition gadgets, measures each
//! distinct piece once on this host in release, and composes the per iteration and
//! the per draw cost from the exact ML DSA 65 multiplicities.
//!
//! It reports the prove time, the verify time, and the proof size for one signing
//! derivation, the average rejection iteration count for the parameter set, the
//! honest additional cost the rejected iterations add on a measured per iteration
//! basis, the lookahead depth the prove time forces in one hundred fifty
//! millisecond slots, and whether the verify time is small enough for the critical
//! path. It does not tune anything.
